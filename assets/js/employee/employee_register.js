$(document).ready(function () {
  ...
  $('#emp_code').on('blur', function () {
    ...
    $.ajax({
      url: `${BASE_URL}employee/ajaxCheckIfEmployeeCodeAlreadyExist/` + input_value,
      type: 'post',
      success: function (response) {
        ...
        if (json[0].Result == 'Ok') {
          ...
          $('#message').html('You can continue with this code');
        } else {
          $('#message').css('margin-left', '651px');
          $('#message').css('position', 'absolute');
          $('#message').css('margin-top', '-20px');
          $('#message').html(json[0].Result);
        }
...
function changestate2() {
  ...
  var x = document.getElementById("StateId1").value;
  $.ajax({
    url: `${BASE_URL}employee/ajaxGetDistrictsByStateId/` + x,
    method: "post",
    success: function (response) {
